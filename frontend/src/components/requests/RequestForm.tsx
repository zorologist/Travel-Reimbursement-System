...
export function RequestForm({ busy, onSubmit }: { busy: boolean; onSubmit: (input: TravelRequestData) => Promise<void> }) {
  ...
  return <form className="request-form-card" onSubmit={(event) => void submit(event)}><div className="request-card-line" /><div className="request-card-body">
    ...
    <section className="request-form-section"><h2>{tr("Travel arrangements", "ترتيبات السفر")}</h2><div className="request-form-grid request-grid-two">
      <label className="request-form-group"><span>{tr("Transportation", "وسيلة الانتقال")}</span><select value={form.transportationMethod} onChange={(event) => change("transportationMethod", event.target.value)}>{transportationOptions.map((option) => <option key={option.value} value={option.value}>{tr(option.english, option.arabic)}</option>)}</select></label>
      ...
      <label className="request-form-group"><span>{tr("Accommodation", "الإقامة")}</span><select value={form.accommodationType} onChange={(event) => change("accommodationType", event.target.value)}>{accommodationOptions.map((option) => <option key={option.value} value={option.value}>{tr(option.english, option.arabic)}</option>)}</select></label>
    </div></section>
